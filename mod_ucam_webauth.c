/* 
   ...
   $Id: mod_ucam_webauth.c,v 1.4 2004-05-04 12:42:43 jw35 Exp $
...
#define APACHE_FNMATCH(pattern, string, flags) ap_fnmatch(pattern, string, flags)
...
#define APACHE_BASE64_DECODE(p, string) (unsigned char *)ap_uudecode(p, string)
#define APACHE_PARSE_HTTP_DATE ap_parseHTTPdate
#define APACHE_PSTRDUP ap_pstrdup
#define APACHE_LOG_ERROR(x, y, rqst, ...) ap_log_rerror(x, y, rqst, __VA_ARGS__)

// DEFINITIONS

#define APACHE_CMD_REC_TAKE1(name, func, data, override, errmsg) {name, func, data, override, TAKE1, errmsg}
...
#define APACHE_FNMATCH(pattern, string, flags) apr_fnmatch(pattern, string, flags)
...
#define APACHE_BASE64_ENCODE(p, string) ap_pbase64encode(p, string)
#define APACHE_BASE64_DECODE(p, string) ap_pbase64decode(p, (const char *)string)
#define APACHE_PARSE_HTTP_DATE(d) apr_date_parse_http(d)
#define APACHE_PSTRDUP apr_pstrdup
#define APACHE_LOG_ERROR(x, y, rqst, ...) ap_log_rerror(x, y, 0, rqst, __VA_ARGS__)
...
#define DEFAULT_AAAuthService     "https://raven.cam.ac.uk/auth/authenticate.html"
...
static int ucam_webauth_handler(request_rec *r);
static void *create_server_config(APACHE_POOL *p, server_rec *s);

static char *get_cgi_param(request_rec *r, char *parm_name);
static void set_cookie(request_rec *r, char *value, mod_ucam_webauth_cfg *c);
static char *SHA1_sign(request_rec *r, mod_ucam_webauth_cfg *c,  char *data);
static int SHA1_sig_verify(request_rec *r, mod_ucam_webauth_cfg *c, char *data, const char *sig);
static int RSA_sig_verify(request_rec *r, char *data, char *sig, char *key_path, char *key_id);
...
static char *cookie_check_sig_string(request_rec *r, APACHE_TABLE *cookie);
static char *wls_response_check_sig_string(request_rec *r, APACHE_TABLE *wls_response);
static char *wls_encode(request_rec *r, char *string);
static char *wls_decode(request_rec *r, char *string);
static char *iso2_time_encode(request_rec *r, APACHE_TIME t);
...
static char *auth_required(request_rec *r);

static const char *set_AAAuthService(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
 
  if (cmd->path != NULL) {
    cfg = (mod_ucam_webauth_cfg *)mconfig;
  } else {
    cfg = (mod_ucam_webauth_cfg *) ap_get_module_config(cmd->server->module_config, &ucam_webauth_module);
  }
...
static const char *set_AADescription(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AAResponseTimeout(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
  ...
  if (cfg->AAResponseTimeout < 0) return "AAResponseTimeout must be a positive number";
  return NULL;
}

static const char *set_AAClockSkew(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AAKeyDir(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AAMaxSessionLife(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
  ...
  if (cfg->AAMaxSessionLife < 0) return "AAMaxSessionLife must be a positive number";
  return NULL;
}

static const char *set_AATimeoutMsg(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AACookieName(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AACookiePath(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AACookieDomain(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AAAuthType(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AAInteract(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AAFail(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AACancelMsg(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const char *set_AANoCookieMsg(cmd_parms *cmd, void *mconfig, const char *arg) {
  mod_ucam_webauth_cfg *cfg;
...
static const command_rec config_commands[] = {
  ...
  APACHE_CMD_REC_TAKE1("AAAuthService", set_AAAuthService, NULL, RSRC_CONF | OR_AUTHCFG,
		       "Raven authentication server logon site"),
  APACHE_CMD_REC_TAKE1("AADescription", set_AADescription, NULL, RSRC_CONF | OR_AUTHCFG,
		       "site description"),
  APACHE_CMD_REC_TAKE1("AAResponseTimeout", set_AAResponseTimeout, NULL, RSRC_CONF | OR_AUTHCFG,
		       "timeout for response messages"),
  APACHE_CMD_REC_TAKE1("AAClockSkew", set_AAClockSkew, NULL, RSRC_CONF | OR_AUTHCFG,
		       "max server/client clock difference"),
  APACHE_CMD_REC_TAKE1("AAKeyDir", set_AAKeyDir, NULL, RSRC_CONF | OR_AUTHCFG,
		       "directory containing WLS keys"),
  APACHE_CMD_REC_TAKE1("AAMaxSessionLife", set_AAMaxSessionLife, NULL, RSRC_CONF | OR_AUTHCFG,
		       "timeout for WAA session"),
  APACHE_CMD_REC_TAKE1("AATimeoutMessage", set_AATimeoutMsg, NULL, RSRC_CONF | OR_AUTHCFG,
		       "message on WAA session timeout"),
  APACHE_CMD_REC_TAKE1("AACookieKey", set_AACookieKey, NULL, RSRC_CONF | OR_AUTHCFG,
		       "key for session cookie"),
  APACHE_CMD_REC_TAKE1("AACookieName", set_AACookieName, NULL, RSRC_CONF | OR_AUTHCFG,
		       "name for session cookie"),
  APACHE_CMD_REC_TAKE1("AACookiePath", set_AACookiePath, NULL, RSRC_CONF | OR_AUTHCFG,
		       "path prefix for session cookie"),
  APACHE_CMD_REC_TAKE1("AACookieDomain", set_AACookieDomain, NULL, RSRC_CONF | OR_AUTHCFG,
		       "domain for session cookie"),
  APACHE_CMD_REC_TAKE1("AAAuthType", set_AAAuthType, NULL, RSRC_CONF | OR_AUTHCFG,
		       "authentication type required"),
  APACHE_CMD_REC_TAKE1("AAInteract", set_AAInteract, NULL, RSRC_CONF | OR_AUTHCFG,
		       "force re-authentication?"),
  APACHE_CMD_REC_TAKE1("AAFail", set_AAFail, NULL, RSRC_CONF | OR_AUTHCFG,
		       "fail at WLS and, don't return?"),
  APACHE_CMD_REC_TAKE1("AACancelMsg", set_AACancelMsg, NULL, RSRC_CONF | OR_AUTHCFG,
		       "custom error for authentication cancelled"),
  APACHE_CMD_REC_TAKE1("AANoCookieMsg", set_AANoCookieMsg, NULL, RSRC_CONF | OR_AUTHCFG,
...
static void *create_dir_config(APACHE_POOL *p, char *path) {
  mod_ucam_webauth_cfg *cfg = (mod_ucam_webauth_cfg *)APACHE_PCALLOC(p, sizeof(mod_ucam_webauth_cfg));
...
/*
static int ucam_webauth_authz(request_rec *r) {
  if (r->main != NULL) {
    return OK;
  }
  ...
  */
  ...
  if (r->method_number == M_POST)
    APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_WARNING, r,
		     "ApacheAA hander invoked for POST request, which it doesn't really support");

  if (c->AAAuthService     == NULL) c->AAAuthService     = server_c->AAAuthService;
  if (c->AADescription     == NULL) c->AADescription     = server_c->AADescription;
  if (c->AAResponseTimeout ==   -1) c->AAResponseTimeout = server_c->AAResponseTimeout;
  if (c->AAClockSkew       ==   -1) c->AAClockSkew       = server_c->AAClockSkew;
  if (c->AAKeyDir          == NULL) c->AAKeyDir          = server_c->AAKeyDir;
  if (c->AAMaxSessionLife  ==   -1) c->AAMaxSessionLife  = server_c->AAMaxSessionLife;
  if (c->AATimeoutMsg      == NULL) c->AATimeoutMsg      = server_c->AATimeoutMsg;
  if (c->AACookieKey       == NULL) c->AACookieKey       = server_c->AACookieKey;
  if (c->AACookieName      == NULL) c->AACookieName      = server_c->AACookieName;
  if (c->AACookiePath      == NULL) c->AACookiePath      = server_c->AACookiePath;
  if (c->AACookieDomain    == NULL) c->AACookieDomain    = server_c->AACookieDomain;
  if (c->AAAuthType        == NULL) c->AAAuthType        = server_c->AAAuthType;
  if (c->AAInteract        == NULL) c->AAInteract        = server_c->AAInteract;
  if (c->AAFail            == NULL) c->AAFail            = server_c->AAFail;
  if (c->AACancelMsg       == NULL) c->AACancelMsg       = server_c->AACancelMsg;
  if (c->AANoCookieMsg     == NULL) c->AANoCookieMsg     = server_c->AANoCookieMsg;
  ...
  if (APACHE_FNMATCH(APACHE_PSTRCAT(r->pool, c->AACookiePath, "*", NULL),
    ...
    APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_ERR, r,
		     "AACookiePath %s is not a prefix of %s", c->AACookiePath, r->parsed_uri.path);
  ...
  if (old_cookie_str != NULL && strcmp(old_cookie_str, TESTSTRING) != 0) {
    ...
    if (SHA1_sig_verify(r, c, cookie_check_sig_string(r, old_cookie), (char *)APACHE_TABLE_GET(old_cookie, "sig"))) {
	...
	return APACHE_SERVER_ERROR;
      ...
      APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_DEBUG, r,
		       "now = %s, issue = %s, expire = %s", iso2_time_encode(r, now), iso2_time_encode(r, issue), iso2_time_encode(r, expire));
	...
	APACHE_REQUEST_USER = (char *)APACHE_TABLE_GET(old_cookie, "principal");
	APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_DEBUG, r,
			 "user = %s", APACHE_REQUEST_USER);
	APACHE_TABLE_ADD(r->subprocess_env, "AAISSUE", APACHE_TABLE_GET(old_cookie, "issue"));
	APACHE_TABLE_ADD(r->subprocess_env, "AAEXPIRE", APACHE_TABLE_GET(old_cookie, "expire"));
	APACHE_TABLE_ADD(r->subprocess_env, "AAID", APACHE_TABLE_GET(old_cookie, "id"));
	APACHE_TABLE_ADD(r->subprocess_env, "AAPRINCIPAL", APACHE_TABLE_GET(old_cookie, "principal"));
	APACHE_TABLE_ADD(r->subprocess_env, "AAAUTH", APACHE_TABLE_GET(old_cookie, "auth"));
	APACHE_TABLE_ADD(r->subprocess_env, "AASSO", APACHE_TABLE_GET(old_cookie, "sso"));
  ...
  if (token_str != NULL) {
    ...
    if (sig_verify_result == 0) {
      ...
      if (strcmp((char *)APACHE_TABLE_GET(response_ticket, "ver"), PROTOCOL_VERSION) != 0) {
	msg = "wrong protocol version in authentication service reply";
	status = "600";
	APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_ERR, r, msg);
      } else if (strcmp(APACHE_TABLE_GET(response_ticket, "status"), "200") != 0) {
	msg = error_message(atoi(APACHE_TABLE_GET(response_ticket, "status")));
	if (APACHE_TABLE_GET(response_ticket, "msg") != NULL) {
	  msg = APACHE_PSTRCAT(r->pool, msg, APACHE_TABLE_GET(response_ticket, "msg"), NULL);
	}
	status = (char*)APACHE_TABLE_GET(response_ticket, "status");
	APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_ERR, r, msg);
      } else {
	APACHE_TIME now = APACHE_TIME_NOW;
	APACHE_TIME issue = iso2_time_decode(r, (char *)APACHE_TABLE_GET(response_ticket, "issue"));
	...
	if (issue < 0) {
	  ...
	  if (issue > now + APACHE_TIME_FROM_SEC(c->AAClockSkew)) {
	    ...
	    status = "600";
	    APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_ERR, r, msg);
	  } else if (now - APACHE_TIME_FROM_SEC(c->AAClockSkew) > issue + APACHE_TIME_FROM_SEC(c->AAResponseTimeout)) {
	    msg = "stale authentication service reply issued at";
	    status = "600";
	    APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_ERR, r,
			     "stale authentication reply issued at %s", APACHE_TABLE_GET(response_ticket, "issue"));
	  } else {
	    APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_INFO, r,
			     "validated WLS token ID %s", APACHE_TABLE_GET(response_ticket, "id"));
	  }
	}
      }
    ...
    session_ticket = APACHE_PSTRCAT(r->pool,
					  ...
					  APACHE_TABLE_GET(response_ticket, "params"), NULL);

    session_ticket = APACHE_PSTRCAT(r->pool, session_ticket, "!1!", SHA1_sign(r, c, session_ticket), NULL);
    ...
    APACHE_TABLE_SET(r->headers_out, "Location", APACHE_TABLE_GET(response_ticket, "url"));
  ...
  if (timeout_msg != NULL) {
    request = APACHE_PSTRCAT(r->pool, request, "&msg=", c->AATimeoutMsg, NULL);
  }
  ...
  if (c->AAClockSkew != 0) {
    request = APACHE_PSTRCAT(r->pool, 
			     request,
			     "&date=", iso2_time_encode(r, APACHE_TIME_NOW),
			     "&skew=", APACHE_PSPRINTF(r->pool, "%d", c->AAClockSkew), NULL);
  }

  request = APACHE_PSTRCAT(r->pool,
			   c->AAAuthService, "?",
			   ap_escape_uri(r->pool, request), NULL);
...
static void set_cookie(request_rec *r, char *value, mod_ucam_webauth_cfg *c) {
  ...
  if (value == NULL) {
    cookie = APACHE_PSTRCAT(r->pool, full_cookie_name(r, c->AACookieName),"= ; path=",
			    c->AACookiePath, "; expires=Thu, 21-Oct-1982 00:00:00 GMT", NULL);
  } else {
    cookie = APACHE_PSTRCAT(r->pool, full_cookie_name(r, c->AACookieName), "=", value,
			    "; path=", c->AACookiePath, NULL);
  }

  if (c->AACookieDomain != NULL) {
    cookie = APACHE_PSTRCAT(r->pool, cookie, "; domain=", c->AACookieDomain, NULL);
  }
...
static char *SHA1_sign(request_rec *r, mod_ucam_webauth_cfg *c,  char *data) {
  unsigned char *new_sig = (unsigned char *)APACHE_PCALLOC(r->pool, EVP_MAX_MD_SIZE + 1);
  unsigned int sig_len;

  APACHE_LOG_ERROR(APLOG_MARK, APLOG_NOERRNO | APLOG_DEBUG, r,
		    "making sig with data = %s", data);

  HMAC(EVP_sha1(), c->AACookieKey, sizeof(c->AACookieKey), (const unsigned char *)data, sizeof(data), new_sig, &sig_len);
...
static int RSA_sig_verify(request_rec *r, char *data, char *sig, char *key_path, char *key_id) {
  ...
  int sig_length;
  int result;
  char *key_full_path = ap_server_root_relative(r->pool, 
...
static APACHE_TABLE *unwrap_wls_token(request_rec *r, char *token_str) {
  ...
  APACHE_TABLE_ADD(wls_token, "msg", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "issue", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "id", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "url", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "principal", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "auth", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "sso", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "life", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "params", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "kid", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
  APACHE_TABLE_ADD(wls_token, "sig", (const char *)ap_getword_nulls(r->pool, &pair, '!'));
...
static int using_https(request_rec *r) {
  return (APACHE_FNMATCH("https*", ap_construct_url(r->pool, r->unparsed_uri, r), 0) != APACHE_FNM_NOMATCH);
}
...
static char *error_message(int err) {
  ...
  case 560 : return "Web server not authorised to use the authentication service";
...
static char *no_cookie(request_rec *r, mod_ucam_webauth_cfg *c) {
  char *cookie_name = ap_escape_html(r->pool, full_cookie_name(r, c->AACookieName));
  char *cookie_domain;
